...
object ByondLinkUtil {
		...
		try {
			database.byondDbConnection.use { connection ->
				connection.prepareStatement(
					...
					return processMemberId(playerStmt, ckey)
				}
			}
	...
	private fun processMemberId(
		playerStmt: PreparedStatement,
		ckey: String
	): YogResult<Snowflake?, String?> {
		...
		try {
			database.byondDbConnection.use { connection ->
				connection.prepareStatement(
					...
					return processCkey(ckeyStmt, snowflake)
				}
			}
		} catch (e: SQLException) {
	...
	private fun processCkey(
		ckeyStmt: PreparedStatement,
		snowflake: Snowflake
	): YogResult<String?, String?> {
