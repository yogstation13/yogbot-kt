...
class UnlinkCommand(discordConfig: DiscordConfig, private val byondConnector: ByondConnector) : TextCommand(
	...
	override fun doCommand(event: MessageCreateEvent): Mono<*> {
		...
		return byondConnector.requestAsync(String.format("?unlink=%s", URLEncoder.encode(args[1], StandardCharsets.UTF_8))).flatMap { requestResult ->
				val message: String = if (requestResult.hasError()) requestResult.error
					?: "Unknown Error" else (requestResult.value as String).replace(
					"\u0000".toRegex(),
					""
				)
				DiscordUtil.reply(event, message)
			}
	}
