...
class StickyRoleManager(
	...
	fun doLogout(member: Member?): Mono<*> {
		...
		return Mono.fromCallable {
			...
			stickyRoleRepository.saveAll(
				member.roleIds.filter { discordConfig.isStickyRole(it.asLong()) }
					.map { StickyRole(member.id.asLong(), it.asLong()) }
			).count()

		}.subscribeOn(Schedulers.boundedElastic()).flatMap { count ->
			if (count > 0) {
				member.privateChannel.flatMap { channel ->
					channel.createMessage(
						"You have left the ${discordConfig.serverName} server with $count sticky " +
							"role${parallelize(count)}. They will be reapplied when you rejoin."
					)
				}.and(
					logChannel.log("**${member.tag}** left the server with $count sticky role${parallelize(count)}")
				)
			} else {
				Mono.empty<Any>()
			}
	...
	fun doLogin(member: Member): Mono<*> {
		return Mono.fromCallable {
			val roles: List<StickyRole> = stickyRoleRepository
				.findAllByDiscordId(member.id.asLong())
				.filter { discordConfig.isStickyRole(it.roleId) }
			...
			if (stickyRoleCount > 0) {
				resultMono =
					resultMono.and(logChannel.log("**${member.tag}** rejoined the server with $stickyRoleCount sticky " +
						"role${parallelize(stickyRoleCount)}"))
			}
			resultMono
		}
	}

	private fun parallelize(c: Int): String {
